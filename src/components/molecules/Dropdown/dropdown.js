      ...
      <div className="dropdown__wrapper " onClick={onClick}>
        <Listbox value={value} onChange={onChange}>
          <Listbox.Button className={`dropdown__listboxButton ${disabled === true ? "bg-[#EAEBEB] cursor-not-allowed" : ""}`}  onClick={onClickButton}>
            {icons && <img className="dropdown__icons" src={icons} alt="dropdown" />}
            <p className="dropdown__value" title={value.name}>
            {value?.name ? value?.name : value?.value || placeholder}
            </p>
            {!notWithArrow && (
              <img
              ...
              {data &&
                data.map((props, idx) => (
                  <Listbox.Option
                    ...
                    title={props?.name ? props.name : props}
                  >
                    {props.name ? props.name : props}
                  </Listbox.Option>
                ))}
            </Listbox.Options>
          )}
