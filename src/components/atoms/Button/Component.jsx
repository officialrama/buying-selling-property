...
function Button({
  ...
  const buttonType = (types) => {
    switch (types) {
      ...
      default:
        return (
          <button disabled={disabled} className={classNames(`basic-button__button-color--${buttonColor}`, { "basic-button__width--full": fullWidth, [`basic-button__padding-size--${paddingSize}`]: paddingSize }, disabled && "cursor-not-allowed", className)} onClick={onClick}>
            <p className={`basic-button__text-style--${textColor} text-primary`}>{isLoading ? <SmallLoading /> : children}</p>
          </button>
        )
    }
