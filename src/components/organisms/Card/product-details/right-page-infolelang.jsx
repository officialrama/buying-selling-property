...
const RightPageInfolelang = ({ hargaProperti, hargaAkhir, gimmickOptions, id, data, allData, dataInputCalc, setDataInputCalc, setShowListPropCompare, bookedStat, soldoutStat }) => {
  ...
  const handleConvertCalc = () => {
    if (!dataInputCalc.lamaPinjaman?.value) {
      ...
      localStorage.setItem("infolelangUtj", "PaidInfolelang");
    ...
    <div>
      <>
        <Button buttonColor="blue" textColor="white" fullWidth={true} paddingSize={"padding-1"}
          ...
          className="mb-3"
          disabled={!hargaProperti || bookedStat === true || soldoutStat === true }
        >
          Ajukan Pembelian
        </Button>
      </>
        ...
        <div>
          ...
          <div>
            ...
            <Button
              ...
              disabled={inputArrPopup?.filter?.(Boolean)?.length !== 3 || bookedStat === true || soldoutStat === true}
            >
              Lihat Kalkulasi
            </Button>
          </div>
...
export default RightPageInfolelang;
