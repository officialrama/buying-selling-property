...
const KprSubmissionSales = ({ data, otherProps, dataInputSalesCalc, setDataInputSalesCalc }) => {
    ...
    useEffect(()=> {
        setDataInputSalesCalc({
            ...
            lt: data?.detailProperti?.lt,
            lb: data?.detailProperti?.lb,
            jenisProperti: data?.alamatProperti?.jenisProperti,
        });
    ...
    useEffect(() => {
      ...
      const handleChangeAlt = (value, name) => {
        const hargaAkhir = parseInt(data?.detailProperti?.hargaAkhir * 5 / 100);
...
export default KprSubmissionSales;