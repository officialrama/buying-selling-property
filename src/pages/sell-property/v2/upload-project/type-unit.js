...
const TypeUnit = ({ editMode, isDeleteProp, setIsDeleteProp, email, listClusterUnit, setListClusterUnit }) => {
  const { id } = useParams();
  const [queryParam] = useSearchParams();
  const [showAddProp, setShowAddProp] = useState(false);
  const { modalDelete, setModalDelete, setId, onFetchData, setOnFetchData } = useListPropertyHooksV2();
    ...
    <div>
      ...
      {showAddProp &&
        <Modal
          ...
          from={window.location.pathname.includes("/edit-project") ? "editProperty" : ""}
          projectId={id}
        />}
      {modalDelete && <Modal
                    ...
                    listClusterUnit?.map((data, index) => {
                      return (
                        <TblListUnit
                          ...
                          onClickEdit={() => {
                            navigate(`/sell-property/v2/upload-project/edit-unit/${encodeURIComponent(data?.id)}?prjId=${encodeURIComponent(data?.projectId)}&clsId=${data?.clusterId === -1 ? null : encodeURIComponent(data?.clusterId)}&group=${encodeURIComponent(data?.groupProperti)}&from=${queryParam?.get("from") === "profile-menu" ? "profile-menu" : "add-property"}`);
                          }}
