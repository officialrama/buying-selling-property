...
import { addKjpp, editKjpp, listKjpp, deleteAdminCabang } from "../../store/actions/fetchData/admin-cabang"
import useRegSalesHooks from "../../hooks/useRegSalesHooks"

const ListKjpp = ({userStatus, email}) => {
    ...
    useEffect(() => {
        dispatch(listKjpp(bodyListOfUser, setDataTemp, "Kjpp"))
        return () => {
            dispatch(saReset());
        }
    }, [bodyListOfUser])
    ...
    const handleModalEdit = (id) => {
        if(id){
            const dataFiltered = dataTemp?.ResponseData?.listKjpp?.filter((e) => e.ID === id)
            setInputsRef({
                id: {value: dataFiltered[0].ID},
                namaKjpp: {isValid: !!dataFiltered[0].Nama, value:dataFiltered[0].Nama},
                addressKjpp: {isValid: !!dataFiltered[0].Deskripsi, value:dataFiltered[0].Deskripsi}
            })
            setProfilePic({
                ...profilePic,
                file: "",
    ...
    return (
        ...
        {state.showSingleModal === true && (
             <Modal
                ...
                onClickDelete={() => {
                    dispatch(deleteAdminCabang(email, referralId, "Kjpp"))
                ...
                <div className="admin-page__srch-drp">
                    ...
                    >
                        <div className="flex flex-row space-x-2 justify-center">
                            ...
                            <div className="whitespace-nowrap">Tambah KJPP</div>
                        </div>
                    </Button>
                </div>
            </div>
...
const AddListKjpp = ({
    ...
    const inputArr = [
        ...
        inputsRef?.namaKjpp?.isValid,
        inputsRef?.addressKjpp?.isValid
    ]
    ...
    return (
        <ModalReferral
            isModal={isModal}
            closeModal={closeModal}
            title={!editMode ? "Tambah KJPP" : "Edit KJPP"}
            editMode={editMode}>
            <form>
                    ...
                    <div class="col-span-2">
                        <div className='p-6 flex-auto'>
                            <div className='mb-4 w-[432px]'>
                                <span className="text-xs font-semibold">Nama KJPP</span>
                                    <TextboxLabel
                                        requiredStar={true}
                                        placeholder="Masukan Nama KJPP"
                                        name="namaKjpp"
                                        value={inputsRef?.namaKjpp?.value}
                                        onChange={memoizedHandleName}
                                        warnText={!inputsRef?.namaKjpp?.value ? "Nama Kjpp tidak boleh kosong" : !inputsRef?.namaKjpp?.isValid ? inputsRef?.namaKjpp?.msgError : ""}
                                    />
                            </div>
                            <div className='mb-4'>
                                <span className="text-xs font-semibold">Alamat</span>
                                    <Textarea
                                        ...
                                        name="addressKjpp"
                                        value={inputsRef?.addressKjpp?.value}
                                        rows="4"
                                        onChange={otherProps.handleInput}
                                        warnText={!inputsRef?.addressKjpp?.value ? "Alamat Kjpp tidak boleh kosong" : !inputsRef?.addressKjpp?.isValid ? inputsRef?.addressKjpp?.msgError : ""}
                                    />
                            </div>
...
const Snackbar = ({ message, timeout = 5000, otherProps }) => {
    ...
    useEffect(() => {
        ...
        otherProps.dispatch(listKjpp(otherProps.bodyListOfUsers, otherProps.setDataTemps, "Kjpp"))
    }, [showSnackbar])
