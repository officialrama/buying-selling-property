...
import { listPendaftaranLRC, exportHotLeadsManagement } from '../../../store/actions/fetchData/hotLeadsManagement';
import TableMIS from '../mis/table-mis';
import { Pagination } from 'flowbite-react';
import moment from "moment";

const ListHLM = ({ userStatus, salesName }) => {
    ...
    useEffect(() => {
        dispatch(listPendaftaranLRC({ ...bodyListOfUser, userType: "superadmin" }, setDataTemp));
        return () => {
            dispatch(saReset());
        };
    }, [bodyListOfUser]);
    ...
    const handleDownload = (e) => {
        dispatch(exportHotLeadsManagement({ ...bodyListOfUser, userType: "superadmin" }, downloadData));
    }
    ...
    return (
        <div>
            ...
            <NavHeaderAdmin />
            <SideMenuAdmin title="MIS Hot Leads Management">
                <div className="admin-page__srch-drp-gmk-wrap gtc-four-column">
                    ...
                    <div className="admin-page__srch-drp">
                            <Button
                                buttonColor="blue"
                                textColor="white"
                                className="mr-2 mobile:text-[12px] text-[15px] mobile:w-full h-full"
                                onClick={() => {
                                    handleDownload();
                                }}
                            >
                ...
                <TableMIS tableType="hotLeadsMgmt" dataTemp={React.useMemo(() => (dataTemp), [dataTemp])} downloadData={downloadData} setDownloadData={setDownloadData} checkProps={{ isCheck, setIsCheck, isCheckAll, setIsCheckAll }} />
