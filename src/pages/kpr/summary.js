...
const SummarySubmitKpr = ({ inputs, dataAddress, dataAddressKTP, waktuKontak, selectedOption }) => {
  moment.locale('id');
  ...
  const detailGimmick = JSON.parse(localStorage.getItem('simulasiCalc'))
  ...
  return (
    ...
    <div className="mobile:hidden flex flex-col gap-3">
      <div className='flex flex-col gap-3 sticky top-16 right-0'>
        ...
        <MenuItems name={"UTJ"} href={"#utj"} active={activeSection === 'utj'} />
        </div>
     </div>
    <div className='flex flex-col gap-4'>
      ...
      <div id="utj" className="p-4 border border-[#D3D4D4] rounded-lg w-[660px]">
        ...
        ) : <FieldSummaryData field="Nominal" value={formatRupiah(0)}/>}
        </div>
      </div>
    </div>
    </div>
...
export default SummarySubmitKpr;